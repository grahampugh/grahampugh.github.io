...
embrace version control where possible, and the ability to write blog posts offline is useful, especially while travelling. I've therefore taken the step to migrate the blog 
...
The [kramdown version of Markdown used with Jekyll][kramdown] does not include the quoting of code snippets using the ``` syntax. Code is highlighted between `~~~ html`...`~~~` tags (`html` can be substituted for `bash`, `yaml`, `markdown`, `python` etc.). If quoting Jekyll Liquid tags (e.g. `{% raw %}{% include google_analytics.html %}{% endraw %}`), one must embed the code in `{% raw %}{% raw ... endraw %}{% endraw %}` tags.
